...
# Replace all posts from source while keeping local section index.
find "$POSTS_DST" -maxdepth 1 -type f -name '*.md' ! -name '_index.md' -delete
cp -f "$POSTS_SRC"/*.md "$POSTS_DST"/
...
done < <(find "$POSTS_DST" -maxdepth 1 -type f -name '*.md' ! -name '_index.md' -print0 | sort -z)
...
find "$POSTS_DST" -maxdepth 1 -type f -name '*.md' ! -name '_index.md' | sort > "$posts_file"
